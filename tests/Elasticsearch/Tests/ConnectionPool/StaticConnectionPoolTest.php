...
namespace TippingCanoeEs\Tests\ConnectionPool;
use TippingCanoeEs;
...
class StaticConnectionPoolTest extends \PHPUnit_Framework_TestCase
{
    ...
    {
        $mockConnection = m::mock('\TippingCanoeEs\Connections\GuzzleConnection')
                          ->shouldReceive('ping')
                          ->andReturn(true)
                          ->getMock()
        ...
        $selector = m::mock('\TippingCanoeEs\ConnectionPool\Selectors\RoundRobinSelector')
                    ->shouldReceive('select')
                    ->andReturn($connections[0])
                    ->getMock();

        $connectionFactory = m::mock('\TippingCanoeEs\Connections\ConnectionFactory');

        $randomizeHosts = false;
        $connectionPool = new \TippingCanoeEs\ConnectionPool\StaticConnectionPool($connections, $selector, $connectionFactory, $randomizeHosts);
    ...
    public function testAllExceptLastHostFailPingRevivesInSkip()
    {
        ...
        $goodConnection = m::mock('\TippingCanoeEs\Connections\GuzzleConnection')
